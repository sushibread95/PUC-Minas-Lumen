...
public class PlayerControllerSystem : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // --- REMOVIDO: A linha que causava o erro (SelectAttack) ---
        // Como usamos Q/E/R, não precisamos mais selecionar slots sequenciais.
    ...
    // Não precisamos mais de OnDestroy para SelectAttack, pois não inscrevemos nada
    ...
    void Update()
    {
        ...
        if (input == null ||
           ...
           (CharacterMenuWindow.Instance != null && CharacterMenuWindow.Instance.menuPanel.activeSelf))
        {
            return;
        }
