...
public class PlayerControllerSystem : MonoBehaviour
{
    ...
    public LockOnSystem lockOn;   // quando em lock, quem gira é o LockOnSystem

    [Header("Move")]
    public float jogSpeed = 4f;           // (não usado)
    ...
    [Range(0f, 1f)] public float walkBlendPoint = 0.5f; // mapeia velocidade -> BlendTree
    ...
    // -------- Spell / Attack --------
    [Header("Spells")]
    [SerializeField] private Spell[] spells;              // struct Spell (definido em Cannon.cs)
    [Header("Components")]
    [SerializeField] private Cannon cannon;               // arraste seu Cannon, ou usa fallback

    // -------- Dodge --------
    [Header("Dodge (Dark Souls style)")]
    [Tooltip("Trigger do Animator para a esquiva/rolamento")] public string dodgeTriggerParam = "DodgeTrigger";
    [Tooltip("Nome do state de dodge (opcional, caso queira sincronizar)")] public string dodgeStateName = "Dodge";
    [Tooltip("Layer do state de dodge")] public int dodgeLayerIndex = 0;
    [Tooltip("Distância percorrida pela esquiva")] public float dodgeDistance = 4f;
    [Tooltip("Duração total da esquiva (s)")] public float dodgeDuration = 0.28f;
    [Tooltip("Cooldown entre esquivas (s)")] public float dodgeCooldown = 0.5f;
    [Tooltip("Curva de velocidade (0..1 tempo → 0..1 velocidade)")] public AnimationCurve dodgeSpeedCurve = AnimationCurve.EaseInOut(0,1,1,0);
    [Tooltip("Tempo de travar movimento durante a esquiva (s)")] public float dodgeLockTime = 0.25f;
    ...
    private InputAction inventoryAction;         // opcional
    private InputAction sprintAction;            // Sprint (Shift / L3)

    private Vector3 velocity;                    
    private bool isGrounded;
    private float moveLockUntil = 0f;            // bloqueia movimento (ex.: durante cast/dodge)
    private Coroutine spellRoutine;              // evita spammar vários casts

    // Toggle state persistente para crouch/sneak/sprint
    private bool _crouchToggled = false;
    private bool _sneakToggled = false;
    private bool _sprintToggled = false; // sprint em toggle
    ...
    void Update()
    {
        ...
        // Sprint como TOGGLE (L3 / Shift)
        bool sprintHeld = false;
        if (sprintAction != null)
        {
            if (sprintAction.WasPressedThisFrame())
                _sprintToggled = !_sprintToggled;  // alterna ligado/desligado
            sprintHeld = _sprintToggled;
        }

        bool attackPressed    = input.Player.AttackB.WasPressedThisFrame();
        bool dodgePressed     = input.Player.Jump.WasPressedThisFrame(); // reutiliza o botão de Jump como Dodge
        bool inventoryPressed = inventoryAction != null && inventoryAction.WasPressedThisFrame();

        // UI / Cursor lock: liberar cursor quando abre inventário/pausa
        if (inventoryPressed) LockCursor(false);

        // DODGE (prioriza sobre movimento/ataque)
        if (dodgePressed && Time.time >= _nextDodgeTime && !_isDodging)
        {
            Vector3 dodgeDir = GetInputDirection(moveInput);
            if (dodgeDir.sqrMagnitude < 0.0001f) dodgeDir = -transform.forward; // sem input → esquiva pra trás
            StartCoroutine(DodgeRoutine(dodgeDir.normalized));
            return; // evita processar o resto neste frame para dar responsividade
        }

        // Movimento base (se não estiver dodging)
        HandleMove(dt, moveInput, crouch, sneak, sprintHeld);

        // --- ATTACK (usa o slot selecionado, NUNCA força slot específico) ---
